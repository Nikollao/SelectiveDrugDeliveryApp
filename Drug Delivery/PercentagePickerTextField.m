...
@implementation PercentagePickerTextField
...
-(NSString *) pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    ...
    return [self.percentages objectAtIndex:row];
}
...
- (void)done{
    ...
    self.selectedObject = [self.percentages objectAtIndex:_selectedRow];
    [self endEditing:YES];
...
-(id) init {
    ...
    if (self) {
        [self pickerDelegate];
        self.percentages = [NSArray arrayWithObjects:@"25 %%",@"50 %%", @"75 %%", @"100 %%",nil];
    }
    return self;
}
